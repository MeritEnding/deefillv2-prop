...
def generator_loss(input, stage1, stage2, neg):
    ...
    return total_gen_loss, gen_hinge_loss, gen_l1_loss, total_ssim_loss
...
def fit(train_ds, epochs, test_ds):
	...
	# Train
        for input_image in tqdm(train_ds):
            ...
            total_gen_loss, gen_hinge_loss, gen_l1_loss, dis_loss,ssim_loss = train_step(input_image, mask)
        ...
        d.append(d_b/count)
        ssim.append(s_b/count)

        check_step = int(checkpoint.step)
        plot_history(g_total, g_hinge, g_l1, d, check_step, ssim)
