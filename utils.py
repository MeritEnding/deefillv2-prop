...
def brush_stroke_mask(FLAGS, name='mask'):
    ...
    def generate_mask(H, W):
        ...
        # 고정된 위치에 브러시 스트로크를 그리도록 수정합니다.
        fixed_x = W // 2  # 고정된 x 좌표
        fixed_y = H // 2  # 고정된 y 좌표
        vertex = [(fixed_x, fixed_y)]  # 고정된 위치에 대한 점 리스트
        for _ in range(np.random.randint(1, 4)):
            ...
            for i in range(num_vertex):
                if i % 2 == 0:
                    angles.append(2 * math.pi - np.random.uniform(mean_angle - angle_range, mean_angle + angle_range))
                else:
                    angles.append(np.random.uniform(mean_angle - angle_range, mean_angle + angle_range))

            # 고정된 위치 주위에 브러시 스트로크를 그립니다.
            for i in range(num_vertex):
                r = np.clip(
