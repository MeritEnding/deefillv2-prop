...
def generator_loss(input, stage1, stage2, neg):
    ...
    total_score = gen_hinge_loss + psnr_score1
    return total_score, gen_hinge_loss, psnr_score1
...
def train_step(input, mask):
  with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
    ...
    total_score, gen_hinge_loss, psnr_score1 = generator_loss(input, stage1, stage2, neg)
  ...
  discriminator_optimizer.apply_gradients(zip(discriminator_gradients,
                                              discriminator.trainable_variables))
  return total_score, gen_hinge_loss, dis_loss, psnr_score1


def fit(train_ds, epochs, test_ds):
    checkpoint.restore(manager.latest_checkpoint)
    if manager.latest_checkpoint:
        g_total, g_hinge, d, psnr = [], [], [], []
    ...
    for ep in trange(epochs):
        ...
        g_total_b, g_hinge_b, d_b, p_b = 0, 0, 0, 0
        count = len(train_ds)
	# Train
        for input_image in tqdm(train_ds):
            mask = create_mask(FLAGS)
            total_score, gen_hinge_loss, dis_loss, psnr_score1 = train_step(input_image, mask)
        ...
        plot_history(g_total, g_hinge, d, psnr ,check_step)
