...
def fid_loss(input,stage1,stage2):
    ...
    total_fid_loss =fid1+fid2

    return total_fid_loss
def generator_loss(input, stage1, stage2, neg):
    ...
    fid_loss1 =fid_loss(input,stage1,stage2)

    total_gen_loss = gen_hinge_loss + gen_l1_loss + fid_loss1
    return total_gen_loss, gen_hinge_loss, gen_l1_loss, fid_loss1
...
def train_step(input, mask):
  with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
    ...
    total_gen_loss, gen_hinge_loss, gen_l1_loss,fid_loss1 = generator_loss(input, stage1, stage2, neg)
  ...
  discriminator_optimizer.apply_gradients(zip(discriminator_gradients,
                                              discriminator.trainable_variables))
  return total_gen_loss, gen_hinge_loss, gen_l1_loss, dis_loss, fid_loss1
...
def fit(train_ds, epochs, test_ds):
	...
	# Train
        for input_image in tqdm(train_ds):
            ...
            total_gen_loss, gen_hinge_loss, gen_l1_loss, dis_loss,fid_loss1 = train_step(input_image, mask)
        ...
        plot_history(g_total, g_hinge, g_l1, d,fid, check_step)
...
#FIT
EPOCHS = 1000 - np.int(checkpoint.step)
