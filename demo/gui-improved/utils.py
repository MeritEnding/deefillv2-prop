...
def generate_images(input, generator, training=True, url=False, num_epoch=0):
    ...
    stage1, stage2, stage3, offset_flow1, offset_flow2 = generator(batch_incomplete, mask, training=training)

    plt.figure(figsize=(30, 30))

    batch_predict = stage3
    ...
    stage3_mask = batch_predict[0] * mask
    ...
    # 교정: 아래 1/2/4/5라인
    display_list = [input[0], batch_incomplete[0], stage1[0], stage2[0], stage3[0], batch_complete[0], offset_flow1[0],
                    offset_flow2[0]]
    title = ['Input Image', 'Input With Mask', 'stage1', 'stage2', 'stage3', 'Inpainted Image', 'Offset Flow1',
             'Offset Flow2']
    if not url:
        for i in range(8):
            plt.subplot(1, 8, i + 1)
...
def brush_stroke_mask(FLAGS, name='mask'):
    ...
    mask = tf.numpy_function(
        generate_mask,
        [height, width, get_coordinates_x(),get_coordinates_y()],  # 중심 좌표 (10, 10)에 타원을 생성합니다.
        tf.float32)
    mask.set_shape([1] + [height, width] + [1])  # 마스크의 형상을 설정합니다.
    return mask  # 타원 모양 마스크를 반환합니다.
